...
class UserValidator {
    public static function chkValidator(array $data) {
        ...
        $patternEmail = "/^[0-9a-zA-Z](?!.*?[\-\_\.]{2})[a-zA-Z0-9\-\_\.]{3,63}@[0-9a-zA-Z](?!.*?[\-\_\.]{2})[a-zA-Z0-9\-\_\.]{3,63}\.[a-zA-Z]{2,3}$/u"; // 이메일 패턴
        $patternPassword = "/^[a-zA-Z0-9!@]{8,20}$/u";  // 패스워드 패턴
        $patternName = "/^[가-힣]{1,50}$/u"; // 이름 패턴
