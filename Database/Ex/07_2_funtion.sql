...
-- 방향은LEADING(좌), BOTH(좌우), TRAILING(우) 지정가능 
...
-- COUNT(컬럼) 검색결과중 해당 컬럼의 값이 NULL이 아닌 레코드의 총수를 출력
SELECT
	COUNT(fire_at) -- null이아닌 레코드개수
	,COUNT(*) -- 모든 레코드개수
FROM employees;
